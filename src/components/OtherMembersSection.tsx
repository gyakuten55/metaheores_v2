    ...
    <section className="bg-gray-800 py-16">
      <div className="container mx-auto px-8">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <button
            onClick={goToPrev}
            className="absolute left-4 z-10 text-white hover:text-gray-300 transition-colors disabled:opacity-30"
          ...
          {/* メンバー画像 */}
          <div className="flex gap-4 overflow-hidden mx-16">
            {otherMembers.slice(currentIndex, currentIndex + visibleCount).map((member) => (
              <Link
                key={member.id}
                to={`/member/${member.id}`}
                className="flex-shrink-0 w-48 h-48 cursor-pointer hover:opacity-80 transition-opacity"
          ...
          <button
            onClick={goToNext}
            className="absolute right-4 z-10 text-white hover:text-gray-300 transition-colors disabled:opacity-30"
